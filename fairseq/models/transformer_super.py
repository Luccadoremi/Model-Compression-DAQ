...
class TransformerSuperModel(SuperFairseqEncoderDecoderModel):
    ...
    def add_args(parser):
        ...
        #For the iPQ
        parser.add_argument('--quant-noise-pq', type=float, default=0.0, help='')
        parser.add_argument('--quant-noise-pq-block-size', type=int, default=8 , help='')
    ...
    def build_model(cls, args, task):
        ...
        def build_embedding(dictionary, embed_dim, path=None, q_noise=0, qn_block_size=8):
            num_embeddings = len(dictionary)
            padding_idx = dictionary.pad()
            emb = Embedding(num_embeddings, embed_dim, padding_idx, q_noise, qn_block_size)
        ...
        if args.share_all_embeddings:
            ...
            encoder_embed_tokens = build_embedding(
                src_dict, args.encoder_embed_dim, args.encoder_embed_path, q_noise, qn_block_size
            )
            ...
            decoder_embed_tokens = build_embedding(
                tgt_dict, args.decoder_embed_dim, args.decoder_embed_path, q_noise, qn_block_size
            )
...
class TransformerDecoder(FairseqIncrementalDecoder):
    ...
    def __init__(self, args, dictionary, embed_tokens, no_encoder_attn=False):
        ...
        self.sample_embed_scale = None
        self.q_noise = getattr(args, "quant_noise_pq", 0)
        self.qn_block_size = getattr(args, "quant_noise_pq_block_size", 8)
    ...
    *args.encoder_normalize_before* to ``True``.
    ...
    def __init__(self, args, layer_idx):
        ...
        self.self_attn = MultiheadAttentionSuper(
            ...
            dropout=args.attention_dropout, self_attention=True, qkv_dim=self.qkv_dim, q_noise=self.q_noise, qn_block_size=self.qn_block_size
        )
        ...
        self.fc1 = quant_noise(LinearSuper(super_in_dim=self.super_embed_dim, super_out_dim=self.super_ffn_embed_dim_this_layer, uniform_=init.uniform_, non_linear='relu'), self.q_noise, self.qn_block_size)
        self.fc2 = quant_noise(LinearSuper(super_in_dim=self.super_ffn_embed_dim_this_layer, super_out_dim=self.super_embed_dim, uniform_=init.uniform_, non_linear='linear'), self.q_noise, self.qn_block_size)
...
class TransformerDecoderLayer(nn.Module):
    ...
    *args.decoder_normalize_before* to ``True``.
    ...
    def __init__(self, args, layer_idx, no_encoder_attn=False, add_bias_kv=False, add_zero_attn=False):
        ...
        self.fc1 = quant_noise(LinearSuper(super_in_dim=self.super_embed_dim, super_out_dim=self.super_ffn_embed_dim_this_layer,
                               uniform_=init.uniform_, non_linear='relu'), self.q_noise, self.qn_block_size)
        self.fc2 = quant_noise(LinearSuper(super_in_dim=self.super_ffn_embed_dim_this_layer, super_out_dim=self.super_embed_dim,
                               uniform_=init.uniform_, non_linear='linear'), self.q_noise, self.qn_block_size)
...
def Embedding(num_embeddings, embedding_dim, padding_idx, q_noise, qn_block_size):
    return quant_noise(EmbeddingSuper(num_embeddings, embedding_dim, padding_idx=padding_idx), q_noise, qn_block_size)
