...
class EM:
    ...
    def initialize_centroids(self):
        ...
        # TODO: buradaki long u kaldırırsak belki assignmentta bi daha çevirmekle uğraşmayız uint8 kullanırız
        self.centroids = self.W[:, indices].t()  # (n_centroids x in_features)

    def step(self, i, dry_run=False):
        """
        ...
        # assignments (E-step)
        if dry_run:
            distances = torch.zeros(self.n_centroids, self.W.shape[1])
    ...
    def resolve_empty_clusters(self):
        ...
        """
        counts = torch.bincount(self.assignments)
        empty_clusters = (counts == 0).nonzero()
        ...
        while len(empty_clusters) > 0:
            ...
            k = empty_clusters[0]  # select first element always
            m = counts.argmax()
